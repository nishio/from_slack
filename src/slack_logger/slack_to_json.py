...
class SlackExtractor:
    ...
    def extract_to_json(self, output_dir: str, year: Optional[int] = None, month: Optional[int] = None, 
        ...
        readme_file = os.path.join(date_range_path, "README.md")
        ...
        oldest_str = oldest_jst.strftime("%Y年%m月%d日 %H:%M:%S")
        latest_str = latest_jst.strftime("%Y年%m月%d日 %H:%M:%S")
        
        total_messages = sum(channel["message_count"] for channel in result["channels"])
        channel_count = len(result["channels"])
        active_channels = [channel for channel in result["channels"] if channel["message_count"] > 0]
        active_channel_count = len(active_channels)
        
        readme_content = f"""# Slack ログ: {start_date_str} から {end_date_str}


- **期間**: {oldest_str} から {latest_str} まで
- **チャンネル数**: {channel_count}件 (メッセージあり: {active_channel_count}件)
...
"""
        ...
        if active_channels:
            readme_content += "### メッセージがあるチャンネル\n\n"
            for channel in sorted(active_channels, key=lambda x: x["message_count"], reverse=True):
                readme_content += f"- **{channel['name']}**: {channel['message_count']}件\n"
            readme_content += "\n"
        
        inactive_channels = [channel for channel in result["channels"] if channel["message_count"] == 0]
        if inactive_channels:
            readme_content += "### メッセージがないチャンネル\n\n"
            for channel in sorted(inactive_channels, key=lambda x: x["name"]):
                readme_content += f"- {channel['name']}\n"
        
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write(readme_content)
        
        print(f"抽出結果の概要を {summary_file} に保存しました")
        print(f"ログ情報を {readme_file} に保存しました")
        return result
...
def main():
    ...
    parser.add_argument('--output-dir', help='出力ディレクトリ', default='./slack_data')
    ...
    parser.add_argument('--auto-join', action='store_true', default=True, 
                        help='公開チャンネルに自動的に参加する（デフォルト: True）')
    ...
    skip_channels = args.skip_channels.split(',') if args.skip_channels else []
